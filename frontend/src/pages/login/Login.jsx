			...
			>
				...
				<div className='mb-6'>
					...
					<p className={`text-xs sm:text-sm mt-1.5 ${isDark ? "text-slate-400" : "text-slate-500"}`}>
						Sign in to resume conversations with your friends
					</p>
				</div>
				...
				<form onSubmit={handleSubmit} className='space-y-4'>
					...
					<div>
						<div className='flex justify-between items-center mb-1.5'>
							...
							<Link
								to='/forgot-password'
								className='text-xs font-medium text-indigo-500 hover:text-indigo-400 hover:underline transition-colors'
							>
								Forgot Password?
							</Link>
						</div>
					...
					<button
						type='submit'
						disabled={loading}
						className='w-full mt-2 py-3 px-4 rounded-xl text-sm font-semibold text-white bg-gradient-to-r from-blue-600 via-indigo-600 to-cyan-500 hover:opacity-95 shadow-lg shadow-indigo-500/25 active:scale-[0.99] transition-all duration-200 flex items-center justify-center gap-2 group border-none'
					>
						{loading ? (
					...
					{/* Link to SignUp */}
					<div className={`text-center pt-3 text-xs sm:text-sm ${isDark ? "text-slate-400" : "text-slate-500"}`}>
